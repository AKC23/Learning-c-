...
bool Has_Solution, anyValueTaken;
...
void Sum_Of_Subsets(int s, int index)
{
    if (index > 0)
    {
        ...
        if (s == sum)
        {
            ...
            if ((index + 1 < Elements) && (vec[index + 1] != 0)) //if next index is not zero
            {
                return;
            }
        }
        // if next index is zero then we should count that also
    }
